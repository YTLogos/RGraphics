...
::: sidebar
更加高级的数据变形操作，特别是数据类型的一致性，方便后续的可视化和建模，引入 tidyverse
:::
...
1. reshape 使用函数 `melt` 和 `cast` 聚合数据（退休） `install.packages("reshape")`
1. reshape2 是 reshape 的继任者，提供两个函数 `melt` 和 `cast` 聚合数据（退休）`install.packages("reshape2")`
1. plyr 统一拆分(split)，计算(apply)，合并(combine)的数据处理流（退休）`install.packages("tidyr")`

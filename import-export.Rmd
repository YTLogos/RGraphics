...
```{sql connection="con",tab.cap = "My Caption", output.var="mtcars34"}
SELECT * FROM mtcars where gear IN (3, 4) and cyl >= ?subjects
```
...
以上代码会将 SQL 的运行结果存在 `data_output` 这是数据库中。如果 SQL 比较长，为了代码美观，把带有变量的 SQL 保存为`.sql`脚本，那怎么在 SQL 的 chunk 中直接导入 SQL 文件

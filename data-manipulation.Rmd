...
subset(iris, Species == "virginica" &
  Sepal.Length > mean(Sepal.Length)) %>% head(5)
# 在行的子集范围内
...
iris[iris$Species == "virginica" &
  iris$Sepal.Length > mean(iris$Sepal.Length), ] %>% head(5)
...
```{r cut,fig.cap="连续型变量分组统计",out.width="35%",echo=TRUE}
# 条形图
...
- `with` 和 `within` 都是进入数据环境，后者会修改数据

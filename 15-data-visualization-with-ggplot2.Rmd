...
我们还想进一步了解这些人是不是就自己开发自己维护，基本没有其他人参与，答案是 Almost Sure. 
...
# 维护多个 R 包的开发者 2559
new_pdb[, "Maintainer"][duplicated(new_pdb[, "Maintainer"])] %>% unique() %>% length()

# 总的开发者 7838
new_pdb[, "Maintainer"][!duplicated(new_pdb[, "Maintainer"])] %>% length()

# 只维护1个R包的开发者 5279
# 第一次提交 R 包
first_ctb <- setdiff(new_pdb[, "Maintainer"][!duplicated(new_pdb[, "Maintainer"])],
        unique(new_pdb[, "Maintainer"][duplicated(new_pdb[, "Maintainer"])]))
```
...
ctb_num <- unlist(
  lapply(
    strsplit(
      subset(new_pdb,
             subset = new_pdb[, "Maintainer"] %in% first_ctb,
             select = "Author"
      ),
      split = ","
    ), length
...
```{r}
# 找到开发者
first_ctb[ctb_num == 37]
# 找到 R 包
subset(new_pdb, subset = grepl("Matt Dowle", new_pdb[, "Maintainer"]), select = "Package")
```

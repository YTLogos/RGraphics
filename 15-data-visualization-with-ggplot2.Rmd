...
R 核心团队除了维护开发 Base R 包以外，还开发了哪些 R 包 

```{r}
pdb_cran <- subset(pdb, subset = grepl(x = pdb[, "Maintainer"], pattern = "(@R-project\\.org)"), 
                   select = c("Package", "Maintainer"))
pdb_cran[order(pdb_cran[, "Maintainer"]), ]
```

这么少，是不是有点意外，比如 Paul Murrell 他的邮箱是 <paul@stat.auckland.ac.nz>
...
pdb_cran <- subset(pdb,
  subset = grepl(
    x = pdb[, "Maintainer"],
    pattern = paste("(", cran_team$name, ")", collapse = "|", sep = "")
  ),
  select = c("Package", "Maintainer", "Published")
)

pdb_cran[, "Maintainer"] <- clean_maintainer(pdb_cran[, "Maintainer"])

pdb_cran[order(pdb_cran[, "Maintainer"], pdb_cran[, "Published"]), ] %>%
  knitr::kable(caption = "R Core Team 维护的 R 包", booktabs = TRUE)
```
...
```{r}
sort(table(pdb_cran[, "Maintainer"]), decreasing = TRUE)
```
...
```{r}
old_pdb <- as.data.frame(pdb[, c("Package", "Published")], stringsAsFactors = FALSE)
# 这天要更新的R包最多
sort(table(pdb[, "Published"]), decreasing = TRUE)[1]

ggplot(old_pdb, aes(as.Date(Published))) +
...
当日发布的R包，不论是新增还是更新之前发布的R包都视为最新版，当日之前的都是旧版本，它们可能存在已经修复的 BUG！这句子好奇怪是吧，因为很多 R 包要么托管在 Github 上，要么托管在 [R-Forge](https://r-forge.r-project.org/) 上开发，而 CRAN 上的版本除了发布日外，一般来讲都会落后。如图所示待更新的 R 包在日期上的分布，有的已经10来年没有更新了，最老的 R 包可以追溯到 `r min(pdb[, "Published"])`，它是 `r old_pdb[which.min(as.Date(old_pdb[, "Published"])), 1]`！！

```{r,eval=FALSE,echo=FALSE}
subset(old_pdb, subset = old_pdb[, "Published"] == min(old_pdb[, "Published"]))
old_pdb[which.min(as.Date(old_pdb[, "Published"])), 1]
```

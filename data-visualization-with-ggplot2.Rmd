...
[RStudio GGplot2 作图插件 esquisse](https://github.com/dreamRs/esquisse)

## Why ggplot2 {#why-ggplot2}

为何使用 ggplot2 [^why-ggplot2]

[优雅的ggplot2](https://pkg.garrickadenbuie.com/gentle-ggplot2)
...
[ggthemes](https://github.com/jrnold/ggthemes) [ggthemr](https://github.com/cttobin/ggthemr)
...
```{r,fig.cap="1973年至2010年世界各地6级及以上地震统计表",echo=TRUE}
data(quake6, package = "MSG")
ggplot(quake6, aes(x = year, y = as.factor(month))) +
  stat_sum(aes(size = ..n..), colour = "lightblue") +
  scale_size(range = c(1, 10), breaks = c(10, 15, 20, 25, 30, 35)) +
  labs(size = "magnitude", x = "Year", y = "Month") +
  theme_bw()
```
...
```{r top-maintainers,fig.cap="(ref:top-maintainers)",echo=TRUE}

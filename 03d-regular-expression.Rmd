...
## 字符串转换 {#character-translations}

`tolower` 将字符串或字符串向量中含有的大写字母全都转化为小写， `toupper` 函数正好与之相反，`chartr` 是对应字符的替换操作.
...
The pattern `(?:...)` groups characters 就是括号，但是不做反向查找

模式 `(?<=...)` and `(?<!...)` 都是反向查找，它们不允许限制符， they do not allow repetition quantifiers nor `\C` in `...`.

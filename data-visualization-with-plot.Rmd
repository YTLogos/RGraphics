...
Base 图形系统的扩展包 [prettyB](https://github.com/jumpingrivers/prettyB)

为了方便记忆函数 `par` 的各个参数，Paul Murrell 整理了一份 [助记符](https://www.stat.auckland.ac.nz/~paul/R/parMemnonics.html)
...
```{r,fig.cap="三角函数添加图例",echo=TRUE,title.mar=TRUE}
...
```{r,fig.cap="设置图例的位置",echo=TRUE,title.mar=TRUE}
xmin <- par("usr")[1]
xmax <- par("usr")[2]
ymin <- par("usr")[3]
ymax <- par("usr")[4]
...
```{r,fig.cap="insert 函数微调图例位置",echo=TRUE,title.mar=TRUE}
plot(y ~ x,
  type = "l", lwd = 3,
  ylab = "", xlab = "angle", main = "Trigonometric functions"
)
...
```{r,fig.cap="将图例放在绘图区域外面",title.mar=TRUE,echo=TRUE}

...
```{r pos,fig.height=4.5,fig.asp=.8}
x <- 1:4
y <- x
...
```{r ISLR,fig.width=5,fig.height=4,fig.cap='条形图'}
...
```{r,echo=TRUE,fig.cap=""}
plot(y~x, cex = (960-z)/(960-690), data = topo, 
...
```{r wrong-scatter, fig.cap="错误的散点图画法",fig.height=4.5,fig.asp=.8}
data("iris")
...
pch 没有向量化，实际只是取了前三个值 16 16 17 对应于 Species 的三类 ，高亮区分某些点 ，关键是让高亮的分界点是由区分意义的

```{r right-scatter,fig.cap="正确的散点图",fig.height=4.5,fig.asp=.8}
data("iris")
stripchart(Petal.Length ~ Species,
...
`densCols` 函数根据点的局部密度生成颜色，密度估计采用核平滑法，由 **KernSmooth** 包的 `bkde2D` 函数实现

参数 colramp 传递一个函数，`colorRampPalette` 根据给定的几种颜色生成函数

参数 bandwidth 实际上是传给 `bkde2D` 函数
...
```{r linear-model,dev='tikz',fig.cap="线性回归模型",out.width="35%",fig.asp=1,fig.width=3.5,fig.show='hold',tikz2png='-quality 100 -density 300',fig.process=to_png,echo=TRUE,eval=identical(.Platform$OS.type,'unix')}
# 带有图标题

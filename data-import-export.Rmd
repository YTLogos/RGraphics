...
### `read.ftable` {#read-write-ftable}


### `readBin` {#read-write-bin}


### `readChar` {#read-write-char}


### `read.dcf` {#read-write-dcf}


### `readClipboard` {#read-write-clipboard}

从粘贴板读取数据

### `read.socket` {#read-write-socket}

## 保存输出结果 {#load-save}

load save 导入数据，也叫读入数据，读取数据，读入到 R 环境中，从磁盘上的文件，变成存储在R环境中的数据对象
...
`sink` 函数将控制台输出结果保存到文件，只将 `outer` 函数运行的结果保存到 `ex-sink.txt` 文件

```{r,echo=TRUE}
sink("data/ex-sink.txt")
i <- 1:10
outer(i, i, "*") # 直积 seq(10) %*% t(seq(10)) 
sink()
```

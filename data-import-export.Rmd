...
```{sql connection="con",tab.cap = "My Caption",max.print=10}
...
如果没有手动配置，会使用默认的参数配置 `host="localhost", port = 9000, db = "default", user = "default", password = "", compression = "lz4"`。现在，我们考虑更加实用的场景，服务器上安装了 Docker，ClickHouse 部署在Docker集群上，如何从本机连接上集群。
...
```{r,eval=is_unix & !is_on_travis,echo=TRUE,cache=FALSE}
library(DBI)
library(RClickhouse)
...
")
```
```{r,cache=FALSE,eval=is_unix & !is_on_travis}
dbDisconnect(conn = con)
```

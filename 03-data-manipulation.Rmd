...
对于有序变量Chick，aggregate 会按照既定顺序返回分组计算的结果
...
在这个例子中 tapply 和 by 的输出结果的表示形式不一样，aggregate返回一个data.frame数据框，tapply返回一个表格table，by 返回特殊的数据类型 by。
...
数据操作，reshape reshape2 plyr dplyr dbplyr sparklyr tidyr

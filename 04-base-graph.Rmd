...
```{r font-sans,echo=TRUE,dev.args=list(family = "sans"),out.width = "45%",fig.width=27/7,fig.cap="默认 sans 字体",fig.asp=1}
par(mar=c(4,4,2,1))
x <- seq(-4, 4, len = 101)
y <- cbind(sin(x), cos(x))
matplot(x, y,
...
for (i in seq(length(nus)))
  lines(x0, besselK(x0, nu = nus[i]), col = ggcols[i], lwd = 2)
# 添加图例
legend("topright",
  legend = paste0("$\\kappa=", rev(nus), "$"),
  col = rev(ggcols), lwd = 2, cex = 0.8
)
# 右图
...
(ref:tikz) tikzDevice 包将 LaTeX 中的 TikZ 绘图引擎引入到基础 R 绘图中


pictex/tikz 都是与 LaTeX 直接相关的图形保存设备

```{r,eval=FALSE,echo=TRUE}
pictex(file = "images/sine.tex", width = 5, height = 4)
sines <- outer(1:20, 1:4, function(x, y) sin(x / 20 * pi * y))
matplot(sines, pch = 1:4, type = "o", col = rainbow( ncol(sines) ),
        xlab = "x", ylab = "y", main = "Sines Function" )
dev.off()
```

在生成的 tex 文件中，可以更改横纵坐标轴标签，图标题，图例内容，尤其当包含数学公式时，这种基于 tex 绘图的方式显得特别灵活，操作也比较简单，表达式不能用在这种 pictex 图形保存设备

```tex
% 保存为 device-pictex.tex
...
%...
\begin{figure}[h]
  \centerline{\input{images/sine.tex}}
  % \caption{sine}
\end{figure}
%...
\end{document}
...
```{r,eval=FALSE,echo=TRUE}
tinytex::xelatex('images/device-pictex.tex')
```

```{r sine-fun,fig.cap="常用的",fig.asp=1,out.width = "45%",fig.width=27/7}
par(mar = c(4, 4, 2, 1))
sines <- outer(1:20, 1:4, function(x, y) sin(x / 20 * pi * y))
matplot(sines,
  pch = 1:4, type = "o", col = rainbow(ncol(sines)),
  xlab = "x", ylab = "y", main = "Sines Function"
)
```
...
```{tikz demo-tikz, fig.cap='tikz 图形'}
...
```{tikz complicated-tikz, fig.cap='稍复杂的 tikz 图形'}
...
```{r linear-model,dev='tikz',fig.cap="线性回归模型",out.width="35%",fig.asp=1,fig.width=3.5,tikz2png='-quality 100 -density 300',fig.process=to_png,eval=.Platform$OS.type=="unix",echo=TRUE}
# 带有图标题
...
```{r bessel,dev='tikz',fig.cap="贝塞尔函数",out.width="35%",fig.asp=1,fig.width=2.5,tikz2png='-quality 100 -density 300',fig.process=to_png,eval=.Platform$OS.type=="unix",echo=TRUE}
x0 <- 2^(-20:10)
nus <- c(0:5, 10, 20)
x <- seq(0, 4, length.out = 501)

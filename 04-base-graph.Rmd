...
先安装 Graphviz^[Graphviz 官网 <http://www.graphviz.org/>]，常用于绘制流程图，广泛用于 tensorflow 和 mxnet 的模型描述中，如图 \@ref(fig:dot-ex) 所示
...
如图\@ref(fig:demo-tikz)所示，模板来自 `system.file('misc', 'tikz2pdf.tex', package = 'knitr')`
...
再来一张稍微复杂的图形，如图 \@ref(fig:complicated-tikz)所示
...
Asymptote 图形如图 \@ref(fig:demo-asymptote) 所示
...
数学公式是这样的，如图\@ref(fig:matplotlib-math)

```{r matplotlib-math, engine = "python",echo = FALSE, fig.cap = "公式",eval=FALSE}
...
plt.show()
#plt.savefig('matplotlib-math.svg')
```

...
1. `dir.exists(paths)`
1. `dir.create(path, showWarnings = TRUE, recursive = FALSE, mode = "0777")`
1. `Sys.chmod(paths, mode = "0777", use_umask = TRUE)`
1. `Sys.umask(mode = NA)`
...
1. `Sys.getlocale(category = "LC_ALL")`
...
`Sys.setFileTime()` 使用系统调用 system call 设置文件或目录的时间
...
`strptime` 用于字符串与 `POSIXlt`、 `POSIXct` 类对象之间的转化，`format` 默认 `tz = ""` 且 `usetz = TRUE` 

...
> RStudio 插件 [remedy](https://github.com/ThinkR-open/remedy) 简化 Markdown 写作

### 语法 {#syntax}

Markdown 基础语法见 RStudio IDE自带的手册 Markdown Quick Reference， 这里主要介绍一下 Markdown 高级语法， 特别是 Pandoc's Markdown，Pandoc 提供了很多对 Markdown 的扩展支持，下面介绍列表、表格、图片和公式的使用
...
```{r knitr-graphics, fig.cap="(ref:text-references)",out.width="45%",echo=FALSE}
knitr::include_graphics(path = "figures/fig11.png")
```

(ref:fig-cap) 测试文本引用
(ref:text-references) 图表标题很长可使用文本引用方式 <https://bookdown.org/yihui/bookdown/markdown-extensions-by-bookdown.html#text-references>
